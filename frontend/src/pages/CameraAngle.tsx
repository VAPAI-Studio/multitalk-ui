...
function CameraAngleSelector({
  ...
  // Helper: convert azimuth degrees to radians
  ...
  useEffect(() => {
    ...
    // Elevation arc (pink tube on the left)
    const arcRadius = 2.4;
    const arcPoints = [];
    for (let i = 0; i <= 32; i++) {
  ...
  }, [isDragging, onAzimuthChange, onElevationChange, snapToElevation]);
...
export default function CameraAngle({ comfyUrl = "" }: Props) {
  const { user } = useAuth();
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
      <div className="flex gap-6 p-6 md:p-10">
        ...
        <div className="flex-1 max-w-4xl space-y-8">
          <div className="text-center space-y-4 py-8">
            ...
            <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto leading-relaxed">
              Upload an image and use the 3D controls to change the camera angle.
            </p>
          </div>
        ...
        <ResizableFeedSidebar
          storageKey="camera-angle"
          config={{
            mediaType: 'all',
            pageContext: ['multi-camera-angle', 'camera-angle'],
